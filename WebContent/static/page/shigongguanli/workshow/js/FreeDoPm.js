...
FreeDoEarth.init=function(earthId,baseImageryProvider)
{
	...
	this.clickedColor=clickColor;	//模型选中颜色
	this.unClickedColor=unClickColor;	//取消选中颜色
...
FreeDoEarth.flyToModel=function(nodeId,callback){
	...
	if(this.pickedModels.length!=0){
		...
		for(var i=0;i<this.pickedModels.length;i++)
			this.pickedModels[i].primitive.color=unClickColor;
	...
	this.pickedModels[0].primitive.color=clickColor;
...
FreeDoEarth.flyToModels=function(nodeIds){
	...
	for(var i=0;i<nodeIds.length;i++){	//设置多模型颜色
		...
		node.primitive.color=clickColor;
	}
